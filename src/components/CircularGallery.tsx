...
// Adapted from React Bits "Circular Gallery" (ogl/WebGL). Customizations:
//   • grayscale shader — only the centered item renders in color
...
const DEFAULT_FONT = 'bold 30px Vazirmatn';
...
// Loads `fontUrl` (a stylesheet or a direct font file) and returns a
// canvas-ready font string. Falls back to `font` on error. When no URL is
// supplied (our case: Vazirmatn is already self-hosted via globals.css), it
// just makes sure the family is ready before the first canvas paint.
...
class Media {
    ...
    this.program = new Program(this.gl, {
      ...
      fragment: `
        ...
        void main() {
          ...
          // Desaturate based on distance from center; centered item stays in color.
          float gray = dot(color.rgb, vec3(0.299, 0.587, 0.114));
          vec3 finalColor = mix(color.rgb, vec3(gray), clamp(uGrayscale, 0.0, 1.0));
  ...
  update(scroll: { current: number; last: number }, direction: 'right' | 'left') {
    ...
    const grayscale = Math.min(Math.max(Math.abs(x) / colorRange, 0), 1);
    this.program.uniforms.uGrayscale.value = grayscale;
